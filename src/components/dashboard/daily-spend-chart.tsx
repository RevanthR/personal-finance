...
type AdHocItem = {
  ...
  category: string | null; customCategory: string | null; ccTemplateId: string | null;
};
...
// Lighten (positive) or darken (negative) a hex color by a percent, so a
// sub-category reads as a shade of its parent category's own color (e.g.
// Personal's blue) instead of an arbitrary unrelated hue.
...
const SHADE_STEPS = [18, -18, 32, -32, 45];
...
function buildCategorySeries(items: AdHocItem[]): Series[] {
  ...
  for (const item of items) {
    const parentCat = item.category ?? "MISCELLANEOUS";
    const key = item.customCategory ?? parentCat;
    if (!totals.has(key)) totals.set(key, { amount: 0, category: parentCat, customCategory: item.customCategory });
  ...
  const series: Series[] = sorted.slice(0, TOP_CATEGORIES).map(([key, v]) => {
    const base = getCategoryColor(v.category, null);
    if (!v.customCategory) return { key, name: getCategoryDisplay(v.category, null), color: base };
    const variant = variantCountByParent.get(v.category) ?? 0;
    variantCountByParent.set(v.category, variant + 1);
    return { key, name: getCategoryDisplay(v.category, v.customCategory), color: shadeColor(base, SHADE_STEPS[variant % SHADE_STEPS.length]) };
  });
...
export function DailySpendChart({ recentMonths, targetMonth, targetYear, todayMonth, todayYear, fmt }: DailySpendChartProps) {
  ...
  const topSubcats = useMemo(() => {
    ...
    for (const item of selected.adHocItems) {
      if (item.type !== "EXPENSE" || !item.customCategory) continue;
      totals.set(item.customCategory, (totals.get(item.customCategory) ?? 0) + item.amount);
    }
